...
const storage=multer.diskStorage({
    destination:(req,file,cb)=>{
        const user=req.user;
        const filePathFolder=`./Scripts/Upload/${user?.userId}/CourseCertificate`;
...
const uploadCourseCertificate=multer({storage:storage,limits:{
    fieldSize:20*1024*1024
},fileFilter:checkPDF});

export default uploadCourseCertificate;