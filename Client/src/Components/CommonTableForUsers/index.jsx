...
export default function CommonTableForUsers({ tableFormat, customerList, status = "", forType=""}) {
  ...
  const handleAddUser = (data, checked) => {
    if (checked) {
      ...
      setSelectedUser((prev) => prev.filter((user) => user !== user));
    }
  };
        ...
        <Table>
            ...
            {customerList.map((customer, index) => (
              <TableRow key={customer._id || index}>
                <TableCell>
                  <Checkbox 
                    disabled={status === ''} 
                    checked={selectedUser.includes(customer)} 
         ...
         ?.reduce((sum, obj) => sum + (obj?.students?.length || 0), 0) || 0}
                ...
                <TableCell className="flex gap-2">
                    {customer?.userRole?.map(item=><Badge className="cursor-pointer bg-green-600 hover:scale-105 hover:bg-blue-600 transition-all ease-in-out">{item}</Badge>)}
                    </TableCell>
                {forType === "teacher" &&
                <TableCell>
                 0
